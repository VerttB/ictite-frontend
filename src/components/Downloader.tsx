...
const exporter = async (path: string ,id?: string, file?: string) => {
    const res = await fetch(`${getBaseUrl()}/${path}/export${id ? `/${id}` : ''}?file=${file}`, {
        method: 'GET',
    ...
    <div>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogClose />
          ...
          <select value={file} onChange={(e) => setFile(e.target.value as 'csv' | 'excel' | 'json')}>
            <option value="csv">CSV</option>
              <option value="excel">Excel</option>
              <option value="json">JSON</option>
            </select>
          <DialogFooter>
            <Button onClick={handleDownload}>Download <DownloadIcon size={16} /></Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
